...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (liveHistorySection) {
        liveHistorySection.style.display = 'block';
    }
});
...
async function initUser() {
    ...
    try {
        ...
        if (response.success) {
            ...
            state.user = { telegram_id: testUserId, first_name: 'Test User', balance: 0, username: 'test_user', photo_url: null };
        }
    } catch (error) {
        state.user = { telegram_id: 999999999, first_name: 'Test User', balance: 0, username: 'test_user', photo_url: null };
    }
    updateUserDisplay();
}
...
function renderCases() {
    ...
    if (state.cases.length === 0) {
        grid.innerHTML = `<div class="empty-state" style="grid-column:1/-1"><div class="empty-state-icon">📦</div><div class="empty-state-text">Пока нет доступных кейсов</div></div>`;
        return;
    }
    ...
    state.cases.forEach(caseItem => {
        ...
        let priceHtml = caseItem.is_free ? 
            `<div class="case-price free" id="card-price-${caseItem.id}">Открыть бесплатно</div>` : 
            `<div class="case-price"><img src="/static/images/star.png" class="price-star-icon" onerror="this.outerHTML='⭐'" alt="star"> ${caseItem.price}</div>`;
...
async function showCasePreview(caseId) {
    ...
    if (caseItem.is_free) {
        ...
        if(priceDisplay) {
            priceDisplay.innerHTML = `<img src="/static/images/star.png" class="price-icon" onerror="this.outerHTML='⭐'" alt="star"><span class="price-amount">${caseItem.price}</span>`;
        }
        if(btnOpenCase) {
            btnOpenCase.disabled = false;
            btnOpenCase.classList.remove('free', 'timer-btn');
            btnOpenCase.innerHTML = `<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M5 12h14M12 5l7 7-7 7"/></svg> Открыть кейс`;
        }
    }
...
async function confirmOpenCase() {
    ...
    if (!state.currentCase.is_free && state.user.balance < state.currentCase.price) {
        ...
        setTimeout(() => showTopupScreen(), 1500);
        return;
    }
    if (state.currentCase.is_free) {
        const canOpen = await checkFreeCaseAvailable();
        if (!canOpen) { showToast('⏰ Бесплатный кейс доступен раз в 24 часа'); return; }
    }
...
function showResult(result) {
    ...
    if (result.gift.is_stars) {
        ...
        if (itemValueSection) itemValueSection.style.display = 'none';
    } else {
        if (btnSell) btnSell.style.display = '';
        if (starsNotice) starsNotice.style.display = 'none';
        if (itemValueSection) itemValueSection.style.display = '';
    }

    const rarityEl = document.getElementById('wonItemRarity');
    if(rarityEl) {
        rarityEl.className = `item-rarity-badge ${result.gift.rarity || 'common'}`;
        rarityEl.textContent = getRarityText(result.gift.rarity || 'common');
    }
}
...
async function sellResultItem() {
    ...
    showLoader();
    const response = await apiRequest('/sell', 'POST', { opening_id: state.currentOpening.opening_id, user_id: state.user.telegram_id });
    hideLoader();
    if (response.success) {
        state.user.balance = response.new_balance;
        updateUserDisplay();
        showToast(`💰 Продано за ${state.currentOpening?.gift?.value || 0} ⭐`);
        closeResultScreen();
    } else {
...
function renderInventory() {
    ...
    if (state.inventory.length === 0) {
        list.innerHTML = `<div class="empty-state"><div class="empty-state-icon"><img src="/static/images/tab-inventory.png" class="empty-state-icon-img"></div><div class="empty-state-text">Инвентарь пуст — открой кейс!</div></div>`;
        return;
    }
    ...
    state.inventory.forEach((item, index) => {
        ...
        let actionsHtml = item.is_withdrawn ? `<div class="inv-done">✅ Выведено</div>` : `
            <div class="inv-actions">
                <button class="btn-inv btn-inv-withdraw" onclick="withdrawInventoryItem(${item.opening_id})"><img src="/static/images/withdraw-icon.png" class="btn-inv-withdraw-icon" onerror="this.outerHTML='📤'">Вывести</button>
                <button class="btn-inv btn-inv-sell" onclick="sellInventoryItem(${item.opening_id}, ${item.gift?.value || 0})"><span class="btn-inv-sell-label">Продать за</span><span class="btn-inv-sell-row"><img src="/static/images/star.png" class="btn-inv-star-icon">${item.gift?.value || 0}</span></button>
            </div>`;
...
async function withdrawInventoryItem(openingId) {
    ...
    hideLoader();
    if (response.success) { showToast('✅ Приз отправляется!'); loadInventory(); } 
    else showToast('❌ ' + (response.error || 'Ошибка вывода'));
}

async function sellInventoryItem(openingId, value) {
    showLoader();
    const response = await apiRequest('/sell', 'POST', { opening_id: openingId, user_id: state.user.telegram_id });
    hideLoader();
    if (response.success) {
        state.user.balance = response.new_balance;
        updateUserDisplay();
...
function renderHistory() {
    ...
    requestAnimationFrame(() => {
        state.history.forEach((item, index) => {
            if (item.gift?.gift_number && item.gift?.gift_number >= 1) renderTGS(`lh_tgs_${index}`, item.gift.gift_number);
        });
    });
}
...
function switchTab(tabName) {
    ...
    const ls = document.querySelector('.live-history-section');
    if (ls) ls.style.display = (tabName === 'cases') ? 'block' : 'none';
}
...
const observer = new MutationObserver(() => {
    ...
    if (mainScreen && mainScreen.classList.contains('active')) tg.BackButton.hide();
...
function shareReferralLink() {
    ...
    if(!input) return;
    const text = encodeURIComponent('🎁 Залетай ко мне! Открывай крутые кейсы в Telegram и получай бонусы!');
